...
function CaseCard({ c, wide = false }: { c: (typeof CASES)[number]; wide?: boolean }) {
  return (
    <article
      className={`group/case relative h-full overflow-hidden border border-seam bg-pit/70 transition-colors duration-500 hover:border-copper/60 ${
        wide ? "lg:grid lg:grid-cols-2" : ""
      }`}
    >
      <div className={`relative overflow-hidden ${wide ? "min-h-64 lg:min-h-0 lg:h-full" : "aspect-[16/10]"}`}>
      ...
      <div className={`p-6 md:p-8 ${wide ? "flex flex-col justify-center" : ""}`}>
        ...
        <dl className="mt-6 grid grid-cols-3 gap-2 sm:gap-3">
          {c.metrics.map(([k, v]) => (
            <div key={k} className="min-w-0 border border-seam bg-ink/60 px-3 py-2.5 transition-colors duration-300 group-hover/case:border-moss">
              <dt className="truncate font-mono text-[9px] tracking-[0.25em] uppercase text-dim">{k}</dt>
      ...
      <div className="mx-auto max-w-7xl px-4 py-24 md:px-8 md:py-36">
        <div className="mb-12 md:mb-16">
          <Rv className="flex items-center gap-4 mb-5">
            ...
            <span className="font-mono text-dim text-xs tracking-[0.3em] uppercase">подтверждено метриками</span>
          </Rv>
          <div className="flex flex-wrap items-end justify-between gap-6">
            ...
            <Rv delay={200} className="font-mono text-xs text-dim">3 из 128 подтверждённых гипотез ↓</Rv>
          </div>
        </div>

        <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
          <Rv className="lg:col-span-7">
            <CaseCard c={CASES[0]} />
          </Rv>
          <Rv delay={120} className="lg:col-span-5">
            <CaseCard c={CASES[1]} />
          </Rv>
          <Rv delay={80} className="lg:col-span-12">
            <CaseCard c={CASES[2]} wide />
          </Rv>
        </div>
      </div>
    </section>
